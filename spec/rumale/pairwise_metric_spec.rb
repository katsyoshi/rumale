...
RSpec.describe Rumale::PairwiseMetric do
  ...
  let(:n_samples_a) { 10 }
  let(:n_samples_b) { 5 }
  let(:samples_a) { Xumo::DFloat.new(n_samples_a, n_features).rand }
  let(:samples_b) { Xumo::DFloat.new(n_samples_b, n_features).rand }
  let(:degree) { 3 }
  let(:gamma) { 0.5 }
  let(:coef) { 1 }

  it 'calculates the pairwise squared errors between different datasets.' do
    dist_mat_bf = Xumo::DFloat.zeros(n_samples_a, n_samples_b)
    ...
    expect(dist_mat.class).to eq(Xumo::DFloat)
  ...
  it 'calculates the euclidean distance matrix of dataset.' do
    dist_mat_bf = Xumo::DFloat.zeros(n_samples_a, n_samples_a)
    n_samples_a.times do |m|
  ...
  it 'calculates the RBF kernel matrix of dataset.' do
    kernel_mat_bf = Xumo::DFloat.asarray(
    ...
    expect(kernel_mat.class).to eq(Xumo::DFloat)
