...
RSpec.describe Rumale::LinearModel::SVR do
  ...
  let(:single_target) { x.dot(Xumo::DFloat[1.0, 2.0]) }
  let(:multi_target) { x.dot(Xumo::DFloat[[1.0, 2.0], [2.0, 1.0]]) }
  let(:n_samples) { x.shape[0] }
  let(:n_features) { x.shape[1] }
  let(:n_outputs) { multi_target.shape[1] }
  let(:fit_bias) { false }
  let(:n_jobs) { nil }
  let(:estimator) { described_class.new(reg_param: 1, epsilon: 0.1, fit_bias: fit_bias, n_jobs: n_jobs, random_seed: 1).fit(x, y) }
