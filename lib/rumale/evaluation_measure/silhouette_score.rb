  ...
  module EvaluationMeasure
    ...
    class SilhouetteScore
      ...
      def score(x, y)
        ...
        intra_dists = Xumo::DFloat.zeros(n_samples)
        ...
        inter_dists = Xumo::DFloat.zeros(n_samples) + Float::INFINITY
        n_clusters.times do |m|
          cls_pos = y.eq(labels[m])
          n_clusters.times do |n|
            next if m == n
            not_cls_pos = y.eq(labels[n])
            inter_dists[cls_pos] = Xumo::DFloat.minimum(
              inter_dists[cls_pos], dist_mat[cls_pos, not_cls_pos].mean(1)
            )
          end
        end

        mask = Xumo::DFloat.ones(n_samples)
        n_clusters.times do |n|
          cls_pos = y.eq(labels[n])
          mask[cls_pos] = 0 unless cls_pos.count > 1
        end

        silhouettes = mask * ((inter_dists - intra_dists) / Xumo::DFloat.maximum(inter_dists, intra_dists))
